...
ols_naive <- data.frame(y,x,w)
ols_naive <- na.omit((ols_naive))
y_ols <- ols_naive$y
x_ols <- ols_naive$x
w_ols <- ols_naive[, !names(ols_naive) %in% c("y", "x")]


sl_lm[[i]] = SuperLearner(Y = y_ols, 
                          X = data.frame(X=x_ols, w_ols, w_ols*x_ols), 
                          family = gaussian(), 
                          SL.library = "SL.lm", 
                          cvControl = list(V=0))

summary(sl_lm[[i]]$fitLibrary$SL.lm_All$object)


####################################################### 
################   Formula for CT and CF   ############
#######################################################
...
gates <- function(Y1, Y2, W1, W2, X1, X2, Q=4, prop_scores=F) {

### Propensity score 
  
if (prop_scores==T) {
sl_w1 = SuperLearner(Y = X1, 
                     X = W1, 
                     newX = W2, 
                     family = binomial(), 
...
cf <- causalForest(CT_fml, data=data.frame(Y=Y1, W1), 
                             treatment=X1, 
                             split.Rule="CT", 
                             split.Honest=T,  
                             split.Bucket=T, 
...
cate_cf <- predict(cf, newdata = W2, type="vector")
...
output[[i]] <- rerun(10, table_from_gates(gates(Y1,Y2,W1,W2,X1,X2))) %>% # Increase reruns in practice!
bind_rows %>%
...
assign(paste(dependent1[i],"GATES",sep="_"),
       ggplot() + geom_errorbar(data=a, 
       mapping=aes(x=coefficient, ymin=ci_lower_90,
       ymax=ci_upper_90), width=0.2, size=1, color="blue") +
       geom_point(data=a, mapping=aes(x=coefficient, y=estimates),
       size=4, shape=21, fill="white"))

ggsave(paste(dependent1[i],"GATE.png",sep="_"), device="png",  plot = last_plot(),
       path = "./")
